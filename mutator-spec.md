...
This document specifies `scripts/gherkin-mutator.cs`, the primary portable
script that mutates Gherkin example values in the parser-defined JSON IR and
runs acceptance test entry points to determine whether those tests detect the
changed specification data. The script uses .NET 10 file-based app
capabilities.
